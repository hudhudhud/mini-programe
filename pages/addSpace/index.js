...
Page({
  ...
  selectUser(){
    let self = this
    let selectdepartIds = self.data.permissionsList.filter(it=>it.accessType==1).map(it=>it.id)
    let selectuserIds = self.data.permissionsList.filter(it=>it.accessType==0).map(it=>it.id)
